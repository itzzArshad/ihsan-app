...
const AdhkarView: React.FC<AdhkarViewProps> = ({ onStreakUpdate }) => {
  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col items-center gap-3 h-[calc(100dvh-130px)] md:h-[800px] min-h-[500px]">
      
      {/* Top Controls Container */}
      <div className="w-full flex flex-col gap-2 px-1 flex-shrink-0">
          ...
          <div className="flex justify-between items-center w-full flex-wrap gap-y-2">
             <div className="flex bg-black/30 p-1 rounded-full border border-white/5 backdrop-blur-md">
                <button onClick={() => setTimeOfDay('Morning')} className={`px-3 py-1.5 rounded-full flex items-center gap-1.5 text-[10px] md:text-xs font-medium transition-all ${timeOfDay === 'Morning' ? 'bg-amber-500/20 text-amber-200 shadow-sm border border-amber-500/30' : 'text-white/40 hover:text-white/60'}`}><Sun size={12} /> Morning</button>
                <button onClick={() => setTimeOfDay('Evening')} className={`px-3 py-1.5 rounded-full flex items-center gap-1.5 text-[10px] md:text-xs font-medium transition-all ${timeOfDay === 'Evening' ? 'bg-indigo-500/20 text-indigo-200 shadow-sm border border-indigo-500/30' : 'text-white/40 hover:text-white/60'}`}><Moon size={12} /> Evening</button>
             </div>
             
             <div className="flex gap-2 items-center">
                 <div className="flex items-center gap-1.5 text-[10px] md:text-xs text-emerald-200/80 font-medium bg-white/5 px-3 py-1.5 rounded-full border border-white/5">
                 ...
                 <div className="flex bg-black/30 p-1 rounded-full border border-white/5 backdrop-blur-md">
                    <button onClick={() => setViewMode('list')} className={`p-1.5 rounded-full transition-all ${viewMode === 'list' ? 'bg-white/10 text-white shadow-sm' : 'text-white/40 hover:text-white/60'}`} title="List View"><ListIcon size={14} /></button>
                    <button onClick={() => setViewMode('card')} className={`p-1.5 rounded-full transition-all ${viewMode === 'card' ? 'bg-white/10 text-white shadow-sm' : 'text-white/40 hover:text-white/60'}`} title="Card View"><LayoutTemplate size={14} /></button>
                 </div>
             </div>
          </div>
          ...
          <div className="w-full px-1">
              <div className="flex justify-between text-[10px] text-white/40 uppercase tracking-widest mb-1">
                 ...
                 <span>{completedIndices.length} / {list.length}</span>
              </div>
              <div className="w-full bg-white/5 rounded-full h-1 overflow-hidden flex items-center">
      ...
      {/* Main Content Area */}
      <div className="relative w-full flex-1 flex flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-[#0F2027] shadow-2xl transition-all duration-500 group min-h-0">
         ...
         {viewMode === 'list' ? (
             // --- LIST VIEW ---
             <div className="relative z-10 flex-1 overflow-y-auto p-3 space-y-2.5 scrollbar-hide">
                 {list.map((item, index) => {
                      ...
                      return (
                          <div 
                              ...
                              className={`group flex items-center gap-3 p-3 rounded-2xl border transition-all cursor-pointer active:scale-[0.98]
                          ...
                          >
                              ...
                              `}>
                                  {isCompleted ? <Check size={14} className="text-white" strokeWidth={3} /> : <span className="text-[10px] font-bold font-mono">x{item.targetCount}</span>}
                              </div>

                              {/* Middle: Content Preview */}
                              <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                                  <div className="flex justify-between items-baseline">
                                      <span className="text-[9px] text-white/30 font-medium uppercase tracking-widest">#{index + 1}</span>
                                      <p className="font-arabic text-right text-base text-white/90 truncate w-full pl-2" dir="rtl">{item.arabicText}</p>
                                  </div>
                                  <p className="text-[10px] text-white/50 truncate font-serif leading-relaxed">{item.englishTranslation}</p>
                              </div>
             ...
             <>
                 {/* Card Header */}
                 <div className="relative z-20 pt-4 px-4 flex justify-between items-center bg-gradient-to-b from-[#0F2027]/80 to-transparent">
                    <button onClick={() => setViewMode('list')} className="p-2 -ml-2 rounded-full hover:bg-white/10 transition-colors text-white/60 flex items-center gap-1 group">
                       <div className="p-1 rounded-full bg-white/5 group-hover:bg-white/10 border border-white/5"><ArrowLeft size={12}/></div>
                       <span className="text-[9px] font-bold uppercase tracking-widest opacity-60">List</span>
                    </button>
                 ...
                 <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 md:px-6 py-2 relative z-10 scrollbar-hide pb-32 md:pb-48">
                     
                     {/* 1. Arabic Section */}
                     <div className="text-center w-full mb-6 relative" dir="rtl">
                         <p 
                             className="font-arabic font-bold text-2xl md:text-4xl text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.3)] py-2 select-none"
                         ...
                         {currentAdhkar.audioUrl && (
                            <div className="flex justify-center mt-4" dir="ltr">
                               <button onClick={toggleAudio} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 hover:bg-white/10 border border-white/5 transition-colors text-emerald-300 text-[10px] uppercase tracking-wider">
                     ...
                     <div className="flex flex-col gap-4 text-center max-w-lg mx-auto">
                        ...
                        {currentAdhkar.transliteration && (
                            <div className="px-4 py-4 rounded-xl bg-white/5 border border-white/5 text-emerald-100/90 text-sm font-medium leading-loose tracking-wide break-words shadow-sm">
                                {currentAdhkar.transliteration}
                            </div>
                        )}

                        {/* 3. Translation/Meaning (Serif Text) */}
                        <p className="text-white/90 text-base md:text-xl font-serif leading-relaxed break-words px-1">
                        ...
                        {currentAdhkar.benefit && (
                            <div className="flex flex-col items-center gap-2 mt-2">
                                ...
                                <div className="flex items-start gap-2 text-amber-200/90 text-xs text-left bg-amber-900/20 p-3 rounded-lg border border-amber-500/20 shadow-sm">
                                   <Sparkles size={14} className="mt-0.5 flex-shrink-0 text-amber-400" />
                        ...
                        {/* 5. Reference */}
                        <p className="text-[9px] md:text-xs text-white/40 uppercase tracking-widest pb-4 break-words">
                            {currentAdhkar.reference}
                        </p>
                     </div>
                 </div>

                 {/* Fixed Bottom Tap Zone - Adjusted Height for Mobile */}
                 <div 
                    className="absolute bottom-0 left-0 w-full z-30 h-24 md:h-32 bg-gradient-to-t from-black via-[#0F2027] to-transparent flex flex-col items-center justify-end pb-0 cursor-pointer select-none active:scale-[0.99] transition-transform touch-manipulation"
                    onClick={handleTap}
                 >
                     {/* Fill Animation Background */}
                     <div className="absolute bottom-0 left-0 w-full h-20 md:h-24 bg-emerald-600/20 origin-bottom transition-all duration-300 ease-out" style={{ transform: `scaleY(${count / currentAdhkar.targetCount})` }}></div>
                     
                     {/* Separator Line */}
                     <div className="w-full h-px bg-gradient-to-r from-transparent via-white/20 to-transparent mb-auto"></div>

                     <div className="relative z-10 w-full h-20 md:h-24 flex items-center justify-center gap-4">
                         {isDone ? (
                            <div className="flex items-center gap-3 animate-in zoom-in duration-300">
                                <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-emerald-500 flex items-center justify-center shadow-[0_0_20px_rgba(16,185,129,0.5)]">
                                    <Check className="text-white" size={16} strokeWidth={4} />
                                </div>
                                <span className="text-lg md:text-xl font-bold text-emerald-100 tracking-tight">Completed</span>
                            </div>
                         ) : (
                            <div className="text-center">
                                <div className="text-4xl md:text-5xl font-mono font-bold text-white drop-shadow-xl tracking-tight leading-none">
                                   {count}<span className="text-xl md:text-2xl text-white/30 ml-1">/{currentAdhkar.targetCount}</span>
                                </div>
                                <span className="text-[8px] md:text-[9px] text-white/40 uppercase tracking-[0.4em] mt-1 block">Tap to Count</span>
                            </div>
                         )}
                     </div>
                 </div>
      ...
      <div className="text-center opacity-40 flex-shrink-0">
